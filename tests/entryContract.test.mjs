...
test('local entry guard runs before dynamic app import', () => {
  ...
  const load = local.indexOf("import('./js/app.js?v=1.6.0-hour-budget-batch-hotfix-7')");
...
test('all active asset cachebusters use the hour budget batch hotfix token', () => {
  ...
  files.forEach(file => {
    ...
    for (const match of source.matchAll(/\?v=(1\.6\.0-[A-Za-z0-9._-]+)/g)) {
      assert.equal(match[1], '1.6.0-hour-budget-batch-hotfix-7', file);
    }
  });
  assert.match(local, /1\.6\.0-hour-budget-batch-hotfix-7/);
});
