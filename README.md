...
目前 source build 為 `1.6.0-forecast-calendar-workflow-hotfix-11`。GAS runtime 使用 collection replace mutation engine；同一 collection 以 250ms debounce 合併，Server 回傳完整 authoritative collection，失敗時前端回復最近一次 confirmed snapshot。
