...
var PTB_ASSET_VERSION = '1.6.0-hour-filter-holiday-salary-layout-hotfix-9';
...
var PTB_REPLACEABLE_COLLECTIONS = Object.freeze({budgets:true,units:true,hourSettings:true,calendarPeriods:true,calendarRows:true,calendarHolidays:true,salaryEntries:true,forecastEvaluations:true});
...
function getGithubPagesBaseUrl_(ctx){var u=getAppConfig_(ctx).githubPagesBaseUrl;if(!u)throw ptbError_('ASSET_URL_NOT_CONFIGURED','尚未設定 PTB_GITHUB_PAGES_BASE_URL');return u;}
function isWriteEnabled_(ctx){return getAppConfig_(ctx).writeMode==='enabled';}
...
function assertWriteEnabled_(ctx){if(!isWriteEnabled_(ctx))throw ptbError_('WRITE_DISABLED','目前未開放寫入');}
