...
asset_version: 1.6.0-hour-filter-holiday-salary-layout-hotfix-9
...
若 `inspectPtb160Schema()` 回報 `03_hour_settings.hourlyWage` 為 deprecated column，必須先審查再於匿名測試 Sheet 執行 `migratePtb160Schema()`。唯讀 plan 會先檢查每筆 Calendar row 的 `sourceHourSettingId`，以及來源 Hour Setting 的 `academicYear`、`unitCode`、`scheduleType` 是否一致；即使 Calendar wage 已為正值也不能略過。任何 source 關聯錯誤都會令 `plan.ok=false`，並在 no-op return、備份及寫入前拒絕。migration 不會自動修改 source ID、年度、單位、類型或歷史時薪。正式 Sheet migration、真實 round-trip 與 deployment 均不在本次執行範圍。

部署測試環境時設定 `PTB_STATIC_ASSET_VERSION=1.6.0-hour-filter-holiday-salary-layout-hotfix-9`。真實 harness 另需匿名測試 Sheet 副本與 `PTB_TEST_MODE=enabled`。
...
Plan token 是兩張來源表 raw headers/values 與兩組 target headers 的 SHA-256。執行時重新讀取並重算，token stale、unresolved wage rows 或 Calendar source integrity blocker 都會在建立備份或寫入前拒絕；schema 已符合 target 時也必須先通過 blocker gate，才可回傳 `writesPerformed:false`。source issue 每列只回報第一個固定優先序 reason：missing source ID、source not found、duplicate source、academic year、unit code、schedule type；診斷僅含 `rowNumber`、`sourceHourSettingId`、`reason`。真正寫入前先備份 `03_hour_settings` 與 `05_calendar_rows`，之後只改這兩張表；post-verify 會重新執行相同 source integrity 分析，並確認 Calendar row count/IDs/source IDs、全部有效時薪、既有正值時薪、Hour Setting row count、Salary 與其餘資料表均保持不變。備份完成後的任何失敗都要嘗試還原兩張表，並以 migration failure 與 rollback failure 的不同錯誤碼回報。

本機匿名候選 XLSX 的唯讀 regression 目前準確偵測 19 筆 `source_unit_code_mismatch`，因此 fixture 維持 `REJECTED`。Google sandbox、migration、real Sheet round-trip 與 deployment 仍為 `NOT_RUN`。
